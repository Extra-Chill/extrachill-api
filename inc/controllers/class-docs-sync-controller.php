...
class ExtraChill_Docs_Sync_Controller {
	...
	public static function sync_doc( $request ) {
		// 1. Verify we are on the correct site (docs site).
		// We use get_current_blog_id() because this is a network-activated plugin.
		// Docs site is ID 10.
		if ( get_current_blog_id() !== 10 ) {
			return new WP_Error(
				'invalid_site',
				'Documentation sync is only allowed on the docs site (ID 10).',
				[ 'status' => 400 ]
			);
		}
