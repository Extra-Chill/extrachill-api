<?php
/**
 * REST route: POST /wp-json/extrachill/v1/content-blocks/ai-adventure
 *
 * AI-powered text adventure endpoint.
 * Delegates to prompt builder in extrachill-content-blocks plugin.
 */
...
add_action( 'extrachill_api_register_routes', 'extrachill_api_register_content_blocks_ai_adventure_route' );

function extrachill_api_register_content_blocks_ai_adventure_route() {
	register_rest_route( 'extrachill/v1', '/content-blocks/ai-adventure', array(
		'methods'             => WP_REST_Server::CREATABLE,
		'callback'            => array( 'ExtraChill_API_Content_Blocks_AI_Adventure', 'handle_request' ),
		'permission_callback' => '__return_true',
	) );
}

class ExtraChill_API_Content_Blocks_AI_Adventure {
	public static function handle_request( WP_REST_Request $request ) {
		...
		$progression_section = ExtraChill_Content_Blocks_Prompt_Builder::build_progression_section( $game_params['progression_history'] );
	...
	private static function ensure_prompt_builder_available() {
		if ( ! defined( 'EXTRACHILL_CONTENT_BLOCKS_PLUGIN_DIR' ) ) {
			return new WP_Error( 'content_blocks_missing', 'Extra Chill Content Blocks plugin is required for AI adventure.', array( 'status' => 500 ) );
		}

		$builder_path = EXTRACHILL_CONTENT_BLOCKS_PLUGIN_DIR . 'src/blocks/ai-adventure/includes/prompt-builder.php';

		// Fall back to build directory if src doesn't exist.
		if ( ! file_exists( $builder_path ) ) {
			$builder_path = EXTRACHILL_CONTENT_BLOCKS_PLUGIN_DIR . 'build/blocks/ai-adventure/includes/prompt-builder.php';
		}
		...
		if ( ! class_exists( 'ExtraChill_Content_Blocks_Prompt_Builder' ) ) {
	...
	private static function handle_introduction_request( $params ) {
		$messages = ExtraChill_Content_Blocks_Prompt_Builder::build_introduction_messages( $params );
	...
	private static function handle_conversation_turn( $params, $progression_section ) {
		$conversation_messages = ExtraChill_Content_Blocks_Prompt_Builder::build_conversation_messages( $params, $progression_section );
	...
	private static function analyze_progression( $params, $progression_section ) {
		$progression_messages = ExtraChill_Content_Blocks_Prompt_Builder::build_progression_messages( $params, $progression_section, $params['triggers'] );
