<?php
/**
 * REST route: POST /wp-json/extrachill/v1/content-blocks/image-voting/vote
 *
 * Image voting endpoint.
...
function extrachill_api_content_blocks_image_voting_vote_handler( $request ) {
	...
	if ( ! function_exists( 'extrachill_content_blocks_process_image_vote' ) ) {
		return new WP_Error(
			'function_missing',
			'Image voting function not available. Please ensure extrachill-content-blocks plugin is activated.',
			array( 'status' => 500 )
		);
	}
